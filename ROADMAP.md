...
> **Status (2026-09-08): Phase 1 complete; Phase 2 through M14 code-complete.** M1–M10 shipped and validated; M11 validated by sample (full corpus generation deliberately stopped — see M11's "Outcome"). Phase 2's polyglot core has landed **M12** (cache `format_version` + named seams), **M13-pre** (feature-layer spike), **M13** (the `StackPack` trait), and **M14** (`RustStack` — `tree-sitter-rust` extraction + module-tree resolution, `detect()` branching, `feature_model() -> None`, validated on a ~35-spec self-corpus with no trait leaks; `FORMAT_VERSION` 3 → 5). Next is **M15** — fold M14's findings, commit the full self-spec corpus, start neutralizing the docs. See "Phase 2 — the polyglot core first".
...
- **M14** (code-complete) — `RustStack` on CodeOwl's own repo. Exercised the seams; no trait shape leaks. `FORMAT_VERSION` 3 → 5.
- **M15** (next) — fold M14's findings into the trait; commit the full self-spec corpus; ship the TS+Rust two-stack checkpoint.
