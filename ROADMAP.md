...
**Deliverable:** `experiments/exp-02-feature-layer.md` (matching the `exp-01` precedent). Conclusions fold into `ARCHITECTURE.md` open question 4 and the M13 / M16 / M17 plans as each starts.
...
- **Package → file reference resolution.** `import com.foo.Bar` → `com/foo/Bar.java` on the Maven source root (`src/main/java`), plus same-package implicit imports and `import static`. No `oxc_resolver`, no alias config; the hard part is star imports (`import com.foo.*`) and resolving an unqualified name against the enclosing package + `java.lang`.
...
- **Multi-module Maven resolution.** Each module has its own `src/main/java` root and its own `pom.xml`; a cross-module import resolves via the reactor. Per-service graphs that compose.
- **Entry-point model — `feature_model() -> Some`.** `enumerate_entry_points` returns heterogeneous kinds: JAX-RS `@Path` + `@GET`/`@POST`/… resource methods, `@Incoming`/`@Outgoing` (or `@Channel`) Kafka listeners, `@Scheduled` jobs, `@GrpcService` methods. `EntryPoint.kind` distinguishes them; the human-facing title is derived per kind (HTTP verb + path, channel name, cron expr).
- **`admits_to_core`.** A Java service's `core` is CDI-shaped, not co-location-shaped: an `@Inject`ed bean the entry-point method reaches, an `@Entity`/Panache repository it queries. The M11 `is_colocated || does_data_work` policy is a Next-ism; this milestone reverse-engineers the Java equivalent against one real repo (expect iterations, as M11 warns).
...
6. **One pack per repo** — `detect()` picks exactly one `StackPack` and errors on ambiguity (a repo that's both a Next app and a Rust service). Multi-pack in one repo is deferred past Phase 2. **quarkus-super-heroes tests this early**: it has a `ui-super-heroes/` Node module inside a Java reactor — `detect()` must pick the Java pack (dominant-language heuristic, or an explicit signal) and not choke.
