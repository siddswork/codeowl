...
| quarkus-super-heroes | `~/dev/openSource/test-repos/quarkus-super-heroes` | Java (Quarkus) | ~7 services, Java 25 | The official Quarkus reference application — a real microservice system (`rest-heroes`, `rest-villains`, `rest-fights`, `event-statistics`, `rest-narration`, `grpc-locations`, plus a UI). Independent services rather than one Maven reactor, so it stresses per-service build-manifest discovery. Its value for CodeOwl is two things the other Java repo doesn't have: annotation-driven structure (Jakarta REST, CDI, Panache) and **cross-service edges carried by string** — `@RegisterRestClient` targets and Kafka `@Incoming`/`@Outgoing` channels — which are the same boundary-resolution shape as M10's SQL `.from()`. Each service ships a README + architecture diagram, so generated specs are checkable against ground truth. |
