...
| full-stack-fastapi-template (`backend/` subtree) | `~/dev/openSource/test-repos/full-stack-fastapi-template/backend` | Python (FastAPI) | ~19 `.py` files (repo root is polyglot — ~86 `.tsx` frontend, out of scope) | **M17 (`PythonStack` + FastAPI) — the second `feature_model() -> Some`, and the milestone that fixes the schema layer.** FastAPI decorator-routed endpoints, `APIRouter` prefix nesting, `Depends()` chains behind `Annotated` aliases, SQLModel `table=True` models + Alembic migrations. Target is the `backend/` subdir — the root would `detect()` as TypeScript. Netflix `dispatch` / `fief` are the alternatives if a repo whose root is the Python app is wanted; `dispatch` is also the reserve stress repo, like `leveldb` for C++. |
| quarkus-super-heroes | `~/dev/openSource/test-repos/quarkus-super-heroes` | Java (Quarkus) | ~120 Java files, Maven reactor (7 services) | **M18 — the feature layer for a Java service.** The official Quarkus reference app: a parent-pom reactor of independently-deployable services (`rest-heroes`, `rest-villains`, `rest-fights`, `event-statistics`, `rest-narration`, `grpc-locations`, + a UI). Exercises multi-module resolution, **heterogeneous entry-point kinds** (JAX-RS `@Path`, `@Incoming`/`@Outgoing` Kafka, `@Scheduled`, `@GrpcService`), the JPA-vs-migrations schema question, and **cross-service edges carried by string** (`@RegisterRestClient`) — the same boundary-resolution shape as M10's `.from()`. Each service ships a README + architecture diagram, so specs are checkable against ground truth. |
...
The pilot repo is load-bearing for Phase 1 (M1–M11) and is the M13 pack-extraction target. Phase 2's polyglot milestones each own one of the others: **M14 → codeowl, M16 → commons-lang, M17 → full-stack-fastapi-template, M18 → quarkus-super-heroes**. `leveldb` isn't on the schedule — it's the C++ case held in reserve. A quick `codeowl extract` smoke pass against each new repo is still worth doing the moment its stack's extractor compiles, ahead of that stack's full milestone.
...
> **Status (2026-09-09): Phase 1 complete; Phase 2 through M16 done (branch `m16-java-stack`, not yet merged).** M1–M10 shipped and validated; M11 validated by sample (full corpus generation deliberately stopped — see M11's "Outcome"). Phase 2's polyglot core has landed **M12** (cache `format_version` + named seams), **M13-pre** (feature-layer spike), **M13** (the `StackPack` trait), **M14** (`RustStack` — `tree-sitter-rust` extraction + module-tree resolution, `detect()` branching, `feature_model() -> None`, validated on a ~35-spec self-corpus with no trait leaks), **M15** (M14's one finding folded in — an inherent `impl Foo` merges into `Foo`, still zero `spec.rs` change; symbol source spans follow a folded method; doc-neutralization first pass; `symbol.rs`/`graph.rs` self-specs re-rendered), and **M16** (`JavaStack` on commons-lang — `tree-sitter-java` extractor, path-suffix + same-package resolution, N-way `detect`, `feature_model() -> None`; validated by a commons-lang dogfood — internal resolution 100 %, spec quality high; headline M19 finding: `get_next_spec_task` returns whole-file source for a folded Java class, over the MCP token limit). `FORMAT_VERSION` 3 → 6 (M16 needs no bump). The exhaustive self-spec corpus (~240 specs + `rollup:src` + `system`) is a deferred follow-up. Two bugs found during M16, tracked separately: a `submit`/`next_task` infinite loop on smelly prose (**fixed, PR #17, merged**) and an oxc_resolver `strip_prefix` bug that breaks TS resolution under a symlinked repo root on macOS (fix after M16). See "Phase 2 — the polyglot core first".

> **Status (2026-09-10): M17 rescoped.** The next milestone is now **M17 — `PythonStack` + FastAPI** on `full-stack-fastapi-template` (Python core + the first non-Next feature model + the schema-layer fix — a symbol-level `is_schema_symbol` hook replacing M10's `.sql`-file rule). Quarkus shifts to **M18**, and the iterate-and-ship milestone to **M19**. Driven by the owner: Python is a higher-value fourth stack than a second Java one, and SQLModel forces the schema seam into the right shape before M18 bolts a second persistence idiom on. See the `2026-09-10` changelog entry at the foot of this file, and open question 8 in `ARCHITECTURE.md`.

> **Status (2026-09-15): M17 shipped and merged (PR #33), self-corpus complete (PR #34).** `PythonStack` + `FastApiFeatureModel` landed across 10 commits — extraction, dotted-module/relative import resolution, the symbol-level `is_schema_symbol` seam (`.sql` moved out of the generic core into `TypeScriptNextStack`), and the second `feature_model() -> Some` (route enumeration, `Depends()`/param-type flow edges, `admits_to_core` = CRUD module ∨ touches-`Schema`). Dogfooded live against the full `full-stack-fastapi-template` repo (not just `backend/`), which found and fixed 4 generic-core bugs (feature-participant assembly on module imports; `get_next_spec_task`/`submit_spec` both collapsing/misattributing several entry points sharing one file — the same shape M18's Quarkus resource classes will hit; a `feature:<slug>` target substituting a sibling's task) — all stack-neutral, all in `mcp.rs`/`spec.rs`/`features.rs`. A subsequent `/code-review` pass (high effort) found 10 more issues, 7 real correctness bugs and 3 simplification/efficiency cleanups, all fixed with TDD. The M17 `ROADMAP.md` validation gate (a human read of generated feature specs against real source, not just infrastructure passing) ran against 4 representative routes and passed. CodeOwl's own self-corpus (Track C, open since before M17) is now complete — 21/21 documents current, 0 missing/stale/smelly. A polyglot design spike (`experiments/exp-03-polyglot.md`, PR #32) was also written and reviewed in this window — its core question (should a language pack work identically as primary or secondary) is resolved on paper, but **the owner's call is to build M18/Quarkus first and the polyglot milestone after**, since Quarkus is the smaller, better-understood addition and further exercises the shared core before a second structural axis (multi-root) is added on top. Next: **M18 — Quarkus**.
...
The polyglot core is **M12 → M13-pre spike → M13 → M14 → M15 → M16 → M17 → M18 → M19**, in order:

> **Planned insert (revised 2026-09-14/15, owner): a polyglot / secondary-language milestone after Quarkus, not before.** The owner's 2026-09-10 call was to work the "one primary stack + secondary-language subtrees" problem (`ARCHITECTURE.md` open question 3, "Secondary-language subtrees") *before* M18/Quarkus; that sequencing is now reversed — Quarkus is the smaller, better-understood addition (already de-risked by M17's "several entry points share one file" fixes, the same shape a JAX-RS resource class hits) and validating the bigger multi-root change afterward against 4 mature stacks is a better test than 3. `full-stack-fastapi-template` (Python `backend/` + React `frontend/`) remains the forcing repo — M17 already lands on its backend. Scope is still not designed (a paper spike first, like M13-pre; see `experiments/exp-03-polyglot.md` for the resolved design questions feeding it). Quarkus **keeps its M18 number**; when the polyglot milestone lands it takes the next open number (currently **M19**, pushing iterate-and-ship to **M20**) — not renumbered here yet, the insert happens with the design, in its own PR.
- **M12** (done) — cache-version fix + named seams; protects everything after it.
- **M13-pre** (done) — paper spike, before M13 freezes the trait: CodeOwl's own feature layer (for M14) + a forward sketch of the Java entry-point model (for M16/M18). → `experiments/exp-02-feature-layer.md`.
...
- **M16** (done, branch not merged) — `JavaStack` on **commons-lang**: a third stack the TS/Rust impls didn't shape, and the first real corpus with `feature_model() -> None`. `tree-sitter-java` extractor; `record`/`@interface` → `Container` (decision 1 resolved); resolution keys on path-suffix FQN + same-package scan, not `pom.xml` (internal resolution came in at 100 %); N-way `detect`. Dogfood validated the zero-feature path and spec quality on 7 shared-code specs. Headline M19 finding: `get_next_spec_task` returns a folded Java class's whole-file source, over the MCP token limit.
- **M17** — **`PythonStack` + FastAPI** on **full-stack-fastapi-template**: Python core (`tree-sitter-python`, path-suffix resolution, relative imports) + the **first non-Next `feature_model() -> Some`** (decorator-routed endpoints, `APIRouter` prefix nesting, `Depends()` chains) + the **schema-layer fix** — a symbol-level `is_schema_symbol` pack hook replacing M10's `.sql`-file rule, with the SQLModel/SQLAlchemy implementation. `.sql` leaves the generic core.
- **M18** — **Quarkus** on **quarkus-super-heroes**: the feature layer for a Java *service* — heterogeneous entry-point kinds, multi-module Maven, the JPA/Panache `is_schema_symbol` impl (the second one — confirms the seam shape), `@RegisterRestClient` cross-service edges.
- **M19** — fold M17/M18's findings; finish stack-neutral docs; ship the polyglot core.

M12/M13 are refactors that must regenerate the Phase 1 corpus **identically** — the *spec files* and `get_spec_coverage` output, not the `.codeowl/` cache (its format is expected to change, see M12). Everything from M14 on is real new stacks. The Java track (commons-lang at M16, Quarkus at M18) was pulled into Phase 2 ahead of the deferred items because a JVM service corpus is a high-value target for this project's owner; Python (M17) was inserted ahead of Quarkus because it is the higher-value fourth stack and its ORM forces the schema seam into shape first. Rust-on-self stays M14 because it validates the seams for near-zero cost.
...
2. **What is a Java entry point?** (forward sketch for M16/M18, so M13's `trait FeatureModel` isn't shaped by Rust + Next alone.) commons-lang (M16): almost certainly `None` — a pure utility library with no runtime entry surface. A Quarkus service (M18): **several kinds** of entry point coexisting in one repo — JAX-RS `@Path` resources, `@Incoming`/`@Outgoing` Kafka listeners, `@Scheduled` jobs, `@GrpcService` methods, `main`. Implication for M13: `EntryPoint` carries a *kind*; a route slug is a Next-ism, not a universal name; `enumerate_entry_points` may need to return heterogeneous kinds.

**Deliverable:** `experiments/exp-02-feature-layer.md`. Conclusions fold into `ARCHITECTURE.md` open question 4 and the M13 / M14 / M16 / M17 / M18 plans as each starts.
...
**Scope:** Fold M14's findings back into `trait StackPack` — the leaks that only a genuinely different stack exposes. Commit CodeOwl's self-spec corpus. First pass at making the docs stack-neutral: rewrite `ARCHITECTURE.md`'s "Extractors" / "Feature specs" sections and `setup/codeowl-generate.md` (it currently says "a page like `app/submit/page.tsx`" — pack-specific examples move behind "your stack's entry points"). This is the **two-stack checkpoint, not the end of the polyglot core** — M16 + M18 add Java, M17 adds Python, and M19 does the final trait iteration once a non-Rust, non-web stack has had its say.
...
- **Package → file reference resolution.** `import com.foo.Bar` → `com/foo/Bar.java` on the source root, plus same-package implicit imports and `import static`. No `oxc_resolver`, no alias config; the hard part is star imports (`import com.foo.*`) and resolving an unqualified name against the enclosing package + `java.lang`. **Key on the `src/main/java` layout convention, not on `pom.xml`** — that way a Gradle repo works for free; only multi-module *discovery* (M18) is build-tool-specific.
...
**Validation:** `codeowl extract` + `serve` on commons-lang; generate a partial corpus (shared-code tier + a sample of module rollups + the system spec); a human read of ~8–10 specs for correctness. Every place the trait still assumes a resolver config, a route-shaped feature, or a `.sql` schema file is an M17 (`.sql`) or M19 (trait) finding. **Done** — `serve` + `/codeowl-generate --all --budget=15` produced 7 high-fan-in file specs, all read and judged accurate; internal resolution 100 %, `feature_model() -> None` composes; findings in the status box (headline: `get_next_spec_task` returns whole-file source for a folded Java class, over the MCP token limit).

---

### M17 — `PythonStack` + FastAPI: the second feature layer, and the schema seam
**Size:** L · **Builds on:** M16

#### In plain terms

Python as the fourth stack, on a real FastAPI service — the `backend/` subtree of `full-stack-fastapi-template`. Carries the most weight of any milestone since M14, for two reasons:

1. **FastAPI is the second `feature_model() -> Some`** — the first that isn't Next.js. The check that "a feature is an entry point plus what it reaches" survives a second web framework, and that `EntryPoint { kind }` / `admits_to_core` weren't quietly shaped by Next alone. (Quarkus — now M18 — is the third, and the first non-web one.)
2. **SQLModel forces the schema layer into the right shape.** commons-lang and the Rust self-corpus have no persistence, so M10's "a table is a `.sql` file" assumption has never been challenged. A FastAPI + SQLModel repo defines tables as Python classes (`class Item(ItemBase, table=True)`) in ordinary code files — there is no `.sql` file to detect. M17 replaces the file-level rule (`SourceKind::Schema` in the generic core) with a symbol-level pack hook, so the "generalize the schema layer" line that was M18's is retired here rather than reverse-engineered later from two hardcoded cases. See `ARCHITECTURE.md` open question 3 (updated) and the new open question 8.

#### Scope

**Python extraction (`src/python.rs`).**
- `tree-sitter-python`. `class` / `def` (module + method) / module-level assignments → `Container` / `Callable` / `Value`. Decorators → `markers`, **with their arguments** (`@router.get("/items/{id}")` verbatim — `java.rs::annotations` already does this for Java, same approach).
- `"""docstrings"""` — first statement of a module / class / function body.
- No `export` keyword: `is_exported` = name doesn't start with `_` (or is listed in `__all__` when present).
- `classify`: `test_*.py` / `*_test.py` / `tests/` / `conftest.py` → `Test`. Alembic `versions/*.py` → `Generated` (schema *changes*, not current state, and import-less).

**Import resolution.**
- `import a.b.c` / `from a.b import x` → path-suffix match against the walked `.py` files (`a/b/c.py`), the rule `java.rs` uses — no `pyproject.toml` parse, no virtualenv inspection.
- Relative imports (`from . import x`, `from ..pkg import y`) resolve against the current package, established by `__init__.py` presence.
- `__init__.py` re-exports: one hop, like a barrel file / `pub use`.

**The schema seam (replaces M10's file-level rule — the reusable part).**
- New pack hook: `fn is_schema_symbol(&self, sym: &ExtractedSymbol) -> bool` (default `false` — a stack with no persistence model). The generic pipeline retags any symbol it returns `true` for as `SymbolKind::Schema`, after `extract_symbols`.
- `PythonStack` for SQLModel/SQLAlchemy: a class with `table=True` in its argument list (SQLModel), or a base of `Base` / `DeclarativeBase` / `declarative_base()` (classic SQLAlchemy). **Not** a bare `SQLModel` / `BaseModel` base — those are request/response schemas. In `full-stack-fastapi-template`, `Item` and `User` (both `table=True`) are tables; `ItemCreate`, `ItemPublic`, `UserRegister`, … are not, despite sharing the `SQLModel` base. `table=True` is the whole signal. Table name follows the class name lowercased unless `__tablename__` is set.
- **The code→table edge is free for the ORM case.** Routes query via `session.exec(select(Item))` with `Item` the imported model *class* — an ordinary resolved reference once `Item` is a `Schema` node, so `get_callers` on the table lists the querying modules with no flow-edge machinery. Fuzzy `FROM (\w+)` / `INTO (\w+)` string extraction is a fallback for raw-SQL repos only; this repo has none.
- `SourceKind::Schema` and the hardcoded `.sql` branch in `lang.rs` are removed. A pack that reads `.sql` (Flyway, a plain dump) declares it via `source_kind` and routes to the existing, already language-neutral `schema::extract_tables` — it just stops being reachable only from the TS pack. `.sql` becomes a pack's declared capability, not a core concept.
- New `ExtractedSymbol` field carrying class/decorator **arguments** (design decision 1), since `table=True` lives there, not in a base name or a marker.

**FastAPI feature model (`feature_model() -> Some`).**
- `enumerate_entry_points`: scan `markers` for `@<x>.get`/`.post`/`.put`/`.patch`/`.delete`/`.websocket` where `<x>` is a `FastAPI()` or `APIRouter()` instance. Kinds `http-route`, `websocket`.
- **Router-prefix resolution — the headline FastAPI-ism.** A prefix can sit on `APIRouter(prefix="/items")` (constructor kwarg, as in `routes/items.py`) **or** `include_router(router, prefix=…)` (as `app/main.py` does with `prefix=settings.API_V1_STR`), and it nests. `extract_flow_edges` picks up `include_router(X, prefix=P)` and the `APIRouter(prefix=P)` kwarg; `resolve_flow_edge` maps router object `X` → its module; the feature model threads the assembled prefix onto that module's routes. Where a prefix is a `settings.X` reference rather than a literal, record it symbolically (`{API_V1_STR}/items/`) — the literal segment is what matters for the slug and title.
- Title: verb + assembled path (`GET /items/{id}`). `id` unique across kinds — kind-prefix if needed, same rule M18/Quarkus needs.
- `admits_to_core`: a file reached from the entry point via import **or** via a `Depends(...)` target, that is a CRUD/repository module (`app/crud.py`) **or** touches a `SymbolKind::Schema` symbol. `Depends()` is often hidden behind a module-level `Annotated` alias (`SessionDep = Annotated[Session, Depends(get_db)]`) — resolve the alias name → its `Depends(X)` → `X`. `Depends()` nests; follow it like Next's rendered-component walk. Expect iteration against the one repo (the Next `core` rule took three passes — M11's warning).
- Pydantic / non-`table=True` SQLModel classes are **not** `core` data work — one-hop stub dependencies.
- **No cross-service edges.** `full-stack-fastapi-template` is one service. `httpx`/`requests`-to-another-service resolution is M18's job (Quarkus `@RegisterRestClient`); a multi-service Python repo would reuse it.

**Target path — a subdirectory, on purpose.** `full-stack-fastapi-template` is a polyglot repo: `backend/` is ~19 Python files, `frontend/src/` is ~86 `.ts`/`.tsx` (Vite + TanStack Router, generated OpenAPI client). Pointed at the repo root, `detect()` would count more TypeScript than Python and pick the *frontend* — and the Next feature model doesn't even fit Vite. So M17's target is `codeowl serve <repo>/backend`. This is a legitimate real-world pattern (point CodeOwl at the service you care about) and worth validating as such — but it also makes this repo the concrete case for the **deferred polyglot-repo question** (`ARCHITECTURE.md` open question — "one primary stack + secondary-language extraction"): the frontend, the generated client, and the frontend→backend call edge are all out of scope here and stay a known gap. If a reviewer wants a repo whose *root* is the Python app, Netflix `dispatch` or `fief` are the alternatives (noted in the test-repo table).

**Not in scope:** the frontend / cross-language edges (above); Celery / ARQ / Dramatiq task functions as entry points (add only if a target repo uses them); Django (a separate stack later if wanted); non-SQLModel/SQLAlchemy ORMs (the seam supports them — a one-function `is_schema_symbol` body each — added when a repo needs it, per `ARCHITECTURE.md` open question 8).

#### Design decisions to resolve during M17

1. **`is_schema_symbol` signature / the new `ExtractedSymbol` field.** SQLModel needs to see the class *argument list* (`table=True`); classic SQLAlchemy needs base-class names; JPA (M18) needs `markers` + superclass + interfaces. Leaning: add `bases: Vec<String>` and reuse `markers` for decorators-with-args; the hook takes `&ExtractedSymbol` and reads whichever it needs. Confirm when the SQLModel check is written.
2. **`Depends()` chain depth.** One hop or transitive? `Depends()` has no co-location bound the way Next's component walk does. Leaning: transitive but capped, cap reported like the cascade cap.
3. **Where router-prefix assembly lives.** A `flow_edge` resolved at graph-build time (like `.from()`), or a feature-model-time walk. Leaning: flow edge — the assembled path lands in the graph, not recomputed per feature.
4. **`tree-sitter-python` crate version.** Must match the workspace `tree-sitter` core version (the same compatibility constraint `tree-sitter-java` / `-rust` / `-sequel` already sit under). Verify before committing the dependency.

#### Validation (TDD — write `tests/python_spec.rs` first)

`codeowl serve ~/dev/openSource/test-repos/full-stack-fastapi-template/backend` (subdir — see "Target path" above). Generate:
- the shared-code tier + a sample of file specs — a human read for extraction and resolution correctness (private fns, decorators-with-args, relative imports, `__init__` re-exports);
- feature specs for ≥3 endpoints across ≥2 routers, including at least one behind a `prefix`ed router and one with a `Depends(get_db)` / `Annotated` alias chain — the read judges whether the assembled path is right and whether "Data touched" names the real SQLModel tables (`item`, `user`);
- the system spec — composes over module rollups + the feature set.

Schema-seam check: `Item` and `User` appear as `SymbolKind::Schema` nodes; `get_callers` on `Item` lists `app/api/routes/items.py` and `app/crud.py`; the pilot (TS+Next) regenerates its schema specs **byte-identical** after `SourceKind::Schema` is removed from the core (`structural_sweep.py` on the pilot is part of the gate). Every place the trait still assumes a `.sql` file, a Next-shaped route, or a single ORM is an M19 finding.

Likely **no `FORMAT_VERSION` bump** — symbols are retagged at build time, the persisted shape is unchanged, and the cache is already keyed on `pack.name()`. Confirm during implementation; bump if `bases` lands on the persisted `Symbol`.

---

### M18 — Quarkus: the feature layer for a Java service
**Size:** L · **Builds on:** M17

**Scope:** Extend `JavaStack` (or add a `QuarkusStack` that composes it) and exercise it on **quarkus-super-heroes** (7-module Maven reactor of independently-deployable services — see the test-repo list). This is where Java gets a feature layer and where the M13-pre "several kinds of entry point" sketch gets implemented. The `feature_model()` seam and the `is_schema_symbol` hook both already exist from M17 — M18 is the second implementation of each (a non-web feature model; the JPA/Panache persistence idiom), which is what confirms their shape.
...
- **Schema — the JPA/Panache `is_schema_symbol` implementation.** M17 built the seam (`fn is_schema_symbol(&self, sym: &ExtractedSymbol) -> bool`, default `false`) and wrote the SQLModel/SQLAlchemy body. M18 writes the Java body: a class carrying `@Entity`, or `extends PanacheEntity`, or `implements PanacheRepository<…>` → `SymbolKind::Schema`. This is the **second** implementation of the hook, the one that proves the symbol-level shape (not a `.sql`-file shape) is right. Flyway/Liquibase migrations (`.sql`/`.xml` under `src/main/resources/db`), if the repo has them, are a `source_kind`-declared `.sql` capability on the Java pack — a follow-up, not a blocker.
- **`@RegisterRestClient` cross-service flow edges.** A `@RegisterRestClient` interface plus its `@Path` is a string-carried edge from one service to another's endpoint — the same shape as M10's `.from("table")` and the pilot's `fetch("/api/…")`. `extract_flow_edges` / `resolve_flow_edge` handle it. (M17's single-service Python repo has no analogue — this is the first cross-service work.)
- **God-class `get_next_spec_task` payload (M16's headline finding — moved here from M19 by the owner, since Java God-classes are where it bites hardest).** A folded Container's `source` is its whole span — for `StringUtils` (~9 k lines) that is 60–420 KB, over the MCP result token limit; the client has to persist and grep it. The class-level `### Summary`/`### Behavior` prompt doesn't need every method body. Fix: for a Container task over some size, send member signatures + docstrings (+ private-helper bodies) in place of the full span. Quarkus resource classes and `PanacheEntity` subclasses are the ones that will hit this.

**Validation:** `codeowl serve` on quarkus-super-heroes; generate feature specs for a sample of endpoints across ≥2 services plus the cross-service edges; a human read judging whether a BA-style "how does *fights* call *heroes*" question is answered. Findings feed M19.

---

### M19 — Iterate the trait; ship the polyglot core
**Size:** M · **Builds on:** M18

**Scope:** Fold M17 + M18's findings back into `trait StackPack` / `trait FeatureModel` — the leaks that only a non-Rust, non-web stack (Python, then a Java service) exposes and that M15 couldn't have seen. (The schema layer is no longer on this list — M17 moved it off the `.sql`-file assumption to the symbol-level `is_schema_symbol` hook, and M18 added the second implementation; what remains is folding any hook-signature findings. The God-class `get_next_spec_task` payload fix moved to M18 — the owner scoped it with the Java revisit, since Java God-classes are where it bites hardest.) Finish the doc neutralization M15 started. Commit the Java corpora (commons-lang partial + quarkus-super-heroes feature sample).
- **`get_spec_coverage` `pending` payload (M16, confirmed live 2026-09-16).** Cap / paginate — 626 files ⇒ 95 KB on commons-lang, over the MCP result limit. Unlike the `get_next_spec_task` payload fix (a *blob*-shaped single-document task, fixed by reducing + hard-capping its content — see `fix-godclass-payload-size`, PR #37), `pending` is genuinely *list*-shaped (an array of items), so **pagination + a cursor is the right mechanism here**, not size reduction — the same real-world trigger applies (a VS Code Copilot Chat session spilled an oversized MCP result to a `content.json` temp file and asked the agent to `read_file` it back, which fails when that tool isn't enabled — a documented Copilot limitation, not an `rmcp`/transport bug). `search_code`'s `matches` on a broad regex across a big repo is a secondary candidate for the same treatment, lower priority since it hasn't been observed failing yet.
...
**Validation:** the pilot (TS+Next), CodeOwl (Rust), commons-lang (Java library), full-stack-fastapi-template (Python service), and quarkus-super-heroes (Java service) all generate current corpora from one binary, with every stack-specific decision inside a `StackPack` impl and nothing leaking into `graph.rs` / `spec.rs` / `index.rs`.
...
8. **`FeatureModel` stays minimal** — M14 (Rust) and M16 (Java library) both return `None`, so `FeatureModel` gets its *second* implementation only at M17 (FastAPI). Two methods, no speculative surface: an interface with one real impl for three milestones will accrete TS+Next assumptions nobody can see. Anything M17 turns out to need is a later (M19) addition, not an M13 guess.
...
Revised again 2026-09-10, **M17 rescoped and everything after it renumbered** (owner's call, before any M17 code): **M17 is now `PythonStack` + FastAPI** on the `backend/` subtree of `full-stack-fastapi-template` — Python core extraction + the **first non-Next `feature_model() -> Some`** + the **schema-layer fix** (a symbol-level `is_schema_symbol` pack hook replacing M10's file-level `SourceKind::Schema` rule, with the SQLModel/SQLAlchemy implementation; `.sql` leaves the generic core). Quarkus shifts to **M18** (it becomes the *second* `is_schema_symbol` impl — JPA/Panache — and the second feature model, which is what confirms both seam shapes). The iterate-and-ship milestone is **M19**; its "generalize the schema layer" line is retired, since M17 does that structurally rather than M19 reverse-engineering it from two hardcoded cases. Rationale: Python is the higher-value fourth stack, and SQLModel forces the schema seam into the right shape *before* a second persistence idiom is bolted on. Test-repo table gains the `full-stack-fastapi-template/backend` row (root is polyglot — ~86 `.tsx` frontend vs ~19 `.py`, so `detect()` would pick TypeScript; the milestone targets the subdir, and the repo is logged as the concrete case for the deferred "secondary-language subtrees" question in `ARCHITECTURE.md` open question 3). New `ARCHITECTURE.md` open question 8 (schema/persistence coverage is a growing list, tiered degradation). `GLOSSARY.md` gains an `is_schema_symbol` entry. Forward-pointing "M18 finding" references from M16 became "M19"; dated changelog entries above are left as written. **Two same-day follow-ons captured as forward notes, not yet renumbered** (each lands with its own design/PR): (a) a **polyglot / secondary-language milestone** inserts before Java — see "Planned insert" in "Phase 2 — the polyglot core first"; (b) the **God-class `get_next_spec_task` payload fix** moved from M19 into the Java milestone (M18/Quarkus), the owner's call since Java God-classes are where it bites. Next: **M17** (Python/FastAPI).

Revised again 2026-09-17, **M18 entry-point model + schema + admits_to_core done, `@RegisterRestClient` scope narrowed, God-class fix shipped early** (branch `m18-quarkus`): `enumerate_entry_points` now returns all four kinds this section named, built one per commit against real `quarkus-quickstarts`/`quarkus-super-heroes` shapes — JAX-RS `@Path`+verb (class-level prefix, per-method sub-path), Kafka `@Incoming`/`@Outgoing` (including same-method dual-annotation and same-class-constant channel-name resolution), `@Scheduled` (keyed on method name, since no path/channel-equivalent identity exists — the schedule expression enriches the title only), and `@GrpcService` (class-level marker, every `public` method on the class is an entry point, no interface-hierarchy walk for `@Override`). Kind-prefixed slugs built in from the first commit as planned. `admits_to_core` (CDI-shaped: `@ApplicationScoped`/`@Singleton`/`PanacheRepository`) and the JPA/Panache `is_schema_symbol` body (`@Entity`/`PanacheEntity`/`PanacheRepository<…>` → `Schema`) are both done. **`@RegisterRestClient` scope narrowed to the false-positive exclusion only** (owner's explicit call: "fix the entry-point bug, skip the flow-edge machinery for now") — a `@RegisterRestClient` interface's methods are excluded from `enumerate_entry_points` (they'd otherwise look like a real server resource), but the cross-service flow edge this section originally scoped is **not built**: the real call chain is three hops (`FightResource → FightService → HeroClient → @RestClient HeroRestClient`) with no single-hop syntax marker the way `fetch("literal")`/`.from("literal")`/`Depends(x)` have, so it would be real call-graph analysis, which this project has repeatedly declined to build. Whether the ordinary resolved-import chain plus the client interface's own Javadoc already carry enough of this to a generating agent is a question for this milestone's dogfood validation, not decided yet. The **God-class `get_next_spec_task` payload fix** shipped early and separately, as an urgent production fix (`fix-godclass-payload-size`, PR #37, merged to `master`, then merged back into this branch) after a real VS Code Copilot Chat user hit it on a Python repo — recalibrated the size threshold from a first, never-validated 30,000 down to 4,000 plus an unconditional 8,000-byte hard cap, made both configurable via `--large-class-bytes`/`--max-spec-task-bytes` flags (explicitly not env vars, per the owner), and fixed two more real bugs in code review (a broken doc link, and the cap itself overrunning its own budget by the truncation marker's length). Multi-module Maven resolution is unverified beyond the pre-M18 read-only smoke test (M16's `JavaStack`, unmodified, already extracted the whole 7-module reactor cleanly with no reactor-modelling needed). **Not yet done:** the dogfood validation itself (generate feature specs across ≥2 services plus a `@RegisterRestClient`-adjacent case, human-read them). Next: the dogfood pass.
