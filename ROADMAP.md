...
> **Planned insert (2026-09-10, owner): a polyglot / secondary-language milestone before Java.** The owner wants the "one primary stack + secondary-language subtrees" problem (`ARCHITECTURE.md` open question 3, "Secondary-language subtrees") worked *before* M18/Quarkus — `full-stack-fastapi-template` (Python `backend/` + React `frontend/`) is the forcing repo, and M17 lands on its backend anyway. Scope is not yet designed (a paper spike first, like M13-pre). When it lands it inserts as **M18**, shifting Quarkus → **M19** and iterate-and-ship → **M20**. Not renumbered here yet — the insert happens with the design, in its own PR.
