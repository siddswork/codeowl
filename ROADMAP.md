...
- **M16** (code in progress) — `JavaStack` on **commons-lang**: a third stack the TS/Rust impls didn't shape, and the first real corpus with `feature_model() -> None`. Extractor + pack + 3-way `detect` in (commits 1–4); `record`/`@interface` → `Container` (decision 1 resolved); resolution keys on path-suffix FQN + same-package scan, not `pom.xml`. Commit 5 (the commons-lang generation dogfood) is the pending validation gate.
...
Revised again 2026-09-09, **M16 extractor + pack + detection in** (branch `m16-java-stack`, commits `385fe4e`…`5699b69`): `src/java.rs` (`tree-sitter-java`), `JavaStack` in `stack.rs`, `lang::detect` restructured from a 2-tuple to an N-candidate count-and-match. Design decision 1's Java-`record` sub-question **resolved** — every named type kind → `Container`, no size test (a record is a restricted `final class`; `Value` would drop DTO-per-file layouts from the corpus). Resolution keys on path-suffix FQN matching + a same-package source scan, not `pom.xml`. `feature_model() -> None`; `extract_flow_edges` empty. `FORMAT_VERSION` unchanged. 200 tests; commons-lang extract smoke clean (14,098 symbols). **Pending: commit 5** — the commons-lang `serve` + `/codeowl-generate` dogfood (the validation gate) and the M18 trait-leak findings. Two bugs found in passing, both tracked outside M16: the `submit`/`next_task` smelly-prose infinite loop (fixed, PR #17) and the oxc_resolver `strip_prefix`-under-symlink bug that breaks TS resolution on macOS (fix after M16, own PR — canonicalize `repo_root`). Next: finish M16 commit 5, then **M17**.
