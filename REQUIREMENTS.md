...
Large brownfield orgs (multi-language: C++, Java, Python, etc.) lack a shared, accurate, up-to-date source of truth about how their codebase actually works. Documentation, where it exists at all, was written once and has since rotted; the code is the only reliable source, and reading it fluently is a skill not everyone on the team has. Teams (devs, BAs, QA, SREs, DevOps) and their LLM coding agents (VS Code, Kiro, Claude Code) each re-derive this understanding independently and repeatedly — burning time, and, for the agents, tokens.

## Goal

A single shared "brain": a structural knowledge graph extracted from the codebase, and — layered on top of it — LLM-authored **specs** at every level (symbol → file → feature → directory → system) describing how the code actually works, in prose a human *and* an agent can read. The specs are committed to git and kept honest by content hashing, so they stay in step with the code instead of rotting. Exposed via MCP so any IDE/CLI agent can query them instead of re-exploring the repo, and (Phase 2) via a web UI so non-technical/non-IDE team members (BAs, QA, SREs) can browse and search them too.
...
The project is deliberately built in two phases. Phase 1 proves the core idea — does recursive, graph-scoped spec generation actually produce documentation that is accurate, stays current as the code changes, and is genuinely useful to both a human reader (a BA asking "how does this work") and an LLM agent (working a task in the repo) — at zero infra cost, before any investment is made in the org-scale version.

> **Status (2026-09):** Phase 1 shipped and was validated by sample on 2026-09-07 (see `ROADMAP.md`, M11 "Outcome" — the value proposition is taken as settled, not still under test). Phase 2 is underway, led by the **polyglot core** (`ROADMAP.md` M12–M19): making the Phase 1 TypeScript coupling pluggable behind a `StackPack` trait and adding Rust, Java, and Python stacks. The org-rollout scope decisions below (multi-repo, hosting) are not yet started.
...
- **Spec content priority**: audience-split by document kind, not one dual-purpose format (see `ARCHITECTURE.md`'s "Spec document format"). File- and directory-level specs are written for Devs, QA, DevOps, and LLM agents — precise and technical (signatures, side effects, error cases, dependencies). **Feature- and system-level specs are BA-primary from Phase 1** — plain-language flow, data touched, business rules — and every document of every kind carries a plain-language summary section. Serving BAs was originally deferred to Phase 2; it was promoted to a Phase 1 must-have once it became clear the BA-facing documents are also the ones that answer "how does this feature work," which is the shape most real tasks take.
...
- **Runtime environment**: the server runs wherever the developer's actual toolchain lives, not tied to any one OS — WSL for a Claude Code-in-WSL setup, inside a devcontainer/DevBox for VS Code or Kiro configured that way, or (aspirationally) directly on native Windows for a developer who works there without a Linux layer. Cross-platform capability — not Linux-only — was a primary constraint on CodeOwl's own implementation stack. **Resolved: CodeOwl is written in Rust**, which serves this best (one self-contained binary per platform, no runtime to install, no native-addon builds on Windows) — see `ARCHITECTURE.md`'s "Implementation stack". *Phase 1 reality (2026-09):* validated on **macOS and WSL2-Ubuntu**; **native Windows was set aside by the owner as not a current target** — revisiting it needs `dunce::canonicalize` for the `\\?\` verbatim prefix plus case-insensitive `RepoIndex.files` lookup (`CLAUDE.md` carry-forward, memory `dev-environment`). The Rust choice still stands on its other two grounds — language genericity via WASM-loadable grammars, and Phase 2 memory footprint.
...
- **Phase 1 success criterion**: a real, committed spec corpus over the pilot repo that holds up to a dual-audience quality bar. This is *not* a go/no-go verdict on whether the project is worth continuing — the value proposition (brownfield documentation readable by humans and agents alike, kept current by hashing) is the premise, not the hypothesis under test. Concretely, three cuts (see `ROADMAP.md`'s M11): a BA answers several feature-shaped questions ("how does artwork submission work?") from the feature and system specs alone, no code; a spot-check of file/symbol specs against source finds no invented behavior and no missing error cases; and the deterministic quality check (`get_spec_coverage`) reports nothing smelly across the corpus. Whether the specs *also* cut an agent's re-exploration and token use is a worthwhile fourth, secondary data point — one `utility/mine.py` run over a couple of real tasks — but not a gate.
...
- **LLM coding agents themselves** — a first-class consumer, not just a proxy for a human: an agent queries CodeOwl instead of re-exploring the repo from scratch, which saves it tokens and time. This is one of the two audiences the specs are written for, co-equal with the human reader — not the project's sole purpose.
...
Spans both phases — "one stack per repo" is a standing scope decision; making it pluggable and adding stacks is the leading Phase 2 work (`ROADMAP.md` M12–M19).

- **One stack per repo.** CodeOwl analyzes a repo as a single *stack* — its primary language plus that toolchain's framework, import-resolution, and schema conventions — behind one pluggable `StackPack` (see `ARCHITECTURE.md` "Extractors", `GLOSSARY.md` "Stacks and the pluggability work"). `detect()` picks the one stack by primary-source-file count and errors on genuine ambiguity. Phase 1 was hard-wired to TypeScript + Next.js + SQL/Supabase; Phase 2 pulls every such assumption behind the trait.
- **Stacks supported / in progress:** TypeScript + Next.js (pilot), Rust, Java (classic, then Quarkus), Python + FastAPI. C++ is a held-in-reserve datapoint, not scheduled.
- **Database schema is a companion, not its own stack.** How application code names a table — a `.sql` file, an ORM model class, an annotation — is a per-stack `is_schema_symbol` hook (`ARCHITECTURE.md` open question 8), not a separate SQL pack. Coverage grows one idiom at a time (SQLModel/SQLAlchemy, then JPA/Panache); a repo whose persistence idiom isn't covered degrades to "no schema layer", logged, rather than failing.
- **Multi-language within one repo** — a dominant language plus a secondary-language subtree (a FastAPI backend + a React frontend in one repo) — is **a planned milestone, not yet in scope** (`ROADMAP.md` "Planned insert"). Until it lands, point CodeOwl at the primary subdirectory. Cross-language edges (a generated API client, `fetch` → route across the two) are deferred past that milestone.
...
- **Granularity**: physical files exist at file-level and above only — never one file per function/symbol. Leaf-level (function/class) specs are collapsed into *sections within* their containing file's spec document. Beyond that, the mapping is deliberately **not** strictly 1:1 with source files, in either direction: some files get no document, and feature specs correspond to no single file or directory at all. A document exists where there's something worth saying — a file spec iff the file has ≥1 top-level function/class (exported or not — corrected 2026-09-25, see `ARCHITECTURE.md`'s granularity rules), a directory rollup iff the directory has ≥2 spec-bearing entries, a feature spec per enumerated entry point. Full rules and rationale in `ARCHITECTURE.md`'s "Spec document format."
- **Structure**:
  ...
  - `docs/specs/_features/<slug>.md` — a feature spec: the cross-cutting "how does X work" narrative, anchored to an entry point rather than to a directory. Necessary because features cut *across* the directory tree, and their load-bearing hops (UI→API via `fetch("/api/…")`, API→DB via table-name literals) are string literals the import graph structurally cannot see — so no directory-shaped document can tell the story.
  - `docs/specs/_index.md` — the system-level spec, at the root.
- **Frontmatter**: each spec file records the source path(s) or participants it covers, the source hash(es) it was generated from, and a hash of its own generated body — the pair used both for cache invalidation and for detecting human edits, so a correction is reconciled on the next regeneration rather than silently overwritten (see `ARCHITECTURE.md`, "Recursive spec generation" → "Human corrections"). File specs carry hashes **per symbol**, not just per file, so editing one function doesn't force regeneration of its every sibling section; the body hash covers only the LLM-written prose, not the lines CodeOwl fills in deterministically.
...
3. **Multi-language within one repo** — is analyzing a dominant language plus a smaller secondary-language subtree (frontend + backend in one repo) in scope, in what form, and when? A milestone is planned (`ROADMAP.md` "Planned insert") but its scope is undesigned — how `detect()` chooses a primary and enumerates secondaries, whether a secondary pack runs a reduced extraction (symbols + rollups, no resolution/features), and whether cross-language edges are ever resolved. See `ARCHITECTURE.md` open question 3 ("Secondary-language subtrees").
