...
- **Exit criteria**: specs are actually meaningful (useful to a human, useful to an agent) and demonstrably reduce token usage/re-exploration before any Phase 2 investment is made. Two gates, not one: the agent-side measurement (exploration-tool count and token estimate drop, and the agent actually reaches for `get_spec` over `Read`/`Grep`), and the human-side one — a feature-shaped question ("how does artwork submission work?") is answerable from the feature spec alone, judged by a human reading it without the code.
