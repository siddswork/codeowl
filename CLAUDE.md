...
6. **Rendered-component `core` expansion** (uncommitted at handoff, or the commit after `245998a`): the blind spot from that pilot session — a feature's `core` was entry file + `fetch()`-reached routes only, missing the client-component subtree (`page.tsx` → `<EvaluationClient/>` → `<EvaluationForm/>` where the real flow lives). Now `features.rs::extract_rendered_components` collects uppercase JSX tags per file (stored on `Graph`, plumbed through `index.rs`); `assemble_participants` resolves each `<Tag/>` to its file via M2 import edges and pulls it into `core` **if** it's co-located with the entry (`is_colocated`) **or** does data work (`file_does_data_work` — has a route literal or `.from()`). Presentational `components/ui/*` primitives stay stub dependencies. Once in `core`, a component's own fetches are followed. `tests/feature_components.rs` + unit tests. Skill gained a belt-and-suspenders note (read indirectly-rendered components). 130 unit + 10 integration green.
