...
8. **Tier-0 cap** (uncommitted at handoff, or the commit after `dac7194`): `SHARED_CODE_FAN_IN = 3` made the shared-code tier ~25 `lib/` files on the pilot, so `--all --budget=15` spent everything there and reached zero features (~15 runs away). `prioritize()` now bounds tier 0 to the top `SHARED_CODE_MAX_FILES = 8` files by fan-in. Two subtleties the pilot forced: (a) the cutoff is computed from the *full `items`* (all spec-bearing files, `current` ones too), not `pending` — anchoring to `pending` just kept promoting the next batch into tier 0 forever; (b) `components/ui/` primitives (`Button` at fan-in 26, `Card` at 28) are excluded via `is_ui_primitive` — high fan-in, useless summary. Pilot shared tier is now exactly the 8 real `lib/` files (`supabase`, `judge-utils`, `constants`, `utils`, `errors`, `api-guards`, `judge-limits`, `data-tools`; cutoff 12). `spec.rs` only. Also: the "missing field `id`" error in that session was the *agent* fumbling a deferred-tool load, not a CodeOwl bug. 134 unit + 10 integration green.

**Next: M11 corpus generation** — user runs `/codeowl-generate --all --budget=N` from a session *inside* the pilot. **Re-point `.mcp.json` + restart the MCP there every time codeowl code changes.** `--all` now reaches features in ~4 budgeted runs, or target directly (`/codeowl-generate feature:<slug>`, `/codeowl-generate lib`). See `ROADMAP.md` M11 "Execution / sequencing". Then the four cuts; BA cut needs the user.
