...
7. **Default-import resolution** (uncommitted at handoff, or the commit after `45ddc92`): #6 was inert on the pilot — `imports.rs` tracked only *named* imports, and React components are default-exported (`import Form from './form'`), so `resolve_rendered_component` found nothing. Now `imports.rs::DefaultImport` + `resolve.rs::resolve_default_imports` (specifier → target file, file-level only, externals dropped) on `Graph` as `resolved_default_imports`; `resolve_rendered_component` checks those first. Validated on the pilot: the evaluate page's `EvaluationFormClient` (co-located) now resolves and joins `core`. `imports.rs` + `resolve.rs` unit tests, `tests/feature_components.rs` fixture switched to default exports. 131 unit + 10 integration green.

**Next: M11 corpus generation** — the user runs `/codeowl-generate --all --budget=N` from a session *inside* the pilot repo (not this repo — no MCP here). **Re-point the pilot's `.mcp.json` at the rebuilt binary and restart the MCP there** every time codeowl code changes. The evaluation feature specs will now be legitimately `stale` (their `core` grew) — regenerate them. See `ROADMAP.md`'s M11 "Execution / sequencing". Then the four cuts; BA cut needs the user.
