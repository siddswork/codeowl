...
5. **`null`-result bug + test-code deprioritization** (uncommitted at handoff, or the commit after `9fb0124`): `get_next_spec_task` returned a bare `null` when exhausted, which Claude Code's client rejects as an invalid `structuredContent` — every natural loop termination looked like an error. Now returns `SpecTaskResponse::Done {}` → `{"kind":"done"}`; skill checks `kind == "done"`. Separately, `spec.rs::is_test_path` (`e2e/`, `__tests__/`, `*.test.*`, `*.spec.*`, `cypress/`, `playwright/`) — test files sort to a tier below rollups (tier 5, before system), test dirs are excluded from `enumerate_modules` (no test rollups, system spec doesn't wait on them), and `file_fan_in` no longer counts test importers. Found by reading a real pilot generation session where `e2e/helpers/api-client.ts` (fan-in 71, all from tests) was outranking every real `lib/` file. 128 unit + 9 integration green.

**Known limitation surfaced in that session (not yet fixed):** a feature's `core_sources` is the entry file + `fetch()`-reached routes only — NOT the client-component subtree the page renders (`<EvaluationFormClient/>` → `<EvaluationForm/>`). The agent has to notice and read those files manually or the feature spec is thin. Real fix = JSX-aware `core` expansion in `assemble_participants` (follow imports that are rendered as elements); it's an M11-or-later feature-layer enhancement.
