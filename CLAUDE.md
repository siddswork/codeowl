...
- **Does search belong in Phase 1 at all?** `tantivy` + ONNX embeddings are specified, but neither is on the path to proving the core hypothesis, and both are real build cost. `search_code` could start as embedded ripgrep (`grep` crate) and gain a real index only when something needs it. Decide before building the storage layer, not after.
- **Trim the MCP tool surface.** Twelve tools is a lot for Phase 1. A plausible minimum is `get_spec`, `get_next_spec_task`, `submit_spec`, `get_spec_coverage`, and `get_callers`/`get_callees` — with `trace_path`, `get_tests_for`, `get_dependencies`, and `search_code` deferred until something actually calls for them. Cutting these is cheap now and expensive once they have consumers.
