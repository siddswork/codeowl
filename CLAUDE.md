...
**Stack-modularization scope — decided (2026-09-07).** The TS+Next coupling (`extract.rs`, `imports.rs`, `resolve.rs`, `features.rs`, one line in `index.rs`) gets made pluggable in increments folded into the milestone plans, not as a standalone milestone: **M10 first part** = doc-mark the four coupled files + dedup the `.tsx`/`.ts` grammar pick (pure labeling, ~30 min), then the SQL work lands ad hoc; **M11** = `src/lang.rs` (free functions, no trait) centralizing the grammar pick / `is_extractable` / resolver extension list + a `detect(root)` that fails fast on an unparseable repo; **Phase 2** = the `LanguagePack` trait with a real second language (the only step that validates the seams). See `ROADMAP.md` "Stack modularization".
