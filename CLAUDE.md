...
**M11 in progress this session (uncommitted at handoff, or the commits after `4970ce8`):**
1. **`src/lang.rs`** — the stack-modularization item. Centralizes everything TS/SQL-specific that was spread across modules: `is_extractable`, `is_schema_file`, `ts_parser` (moved from the now-deleted `parse.rs`), `RESOLVER_EXTENSIONS` (was inline in `resolve.rs`), `extract_symbols` (the `.sql`-vs-TS dispatch, was duplicated in `graph.rs` + `index.rs`), and `detect(root)` — fail-fast at startup (`main.rs`, both commands) if a repo has no `.ts/.tsx/.sql`. Free functions, no trait — that's Phase 2. 123 unit + 9 integration green.
2. **Data-touched participant wiring** (the piece M10 deferred): `Participants` gains a `data: Vec<String>` tier — SQL tables the core code queries via a resolved `.from()`. Hashed in `current_participant_hashes` (by `source_hash` — a table has no interface_hash), passed to the feature task as `FeatureTask.data` / `TableContext { id, columns }`. Effect: a feature spec's "Data touched" now has graph backing + goes stale on a schema change to a table it touches, and the agent gets real column lists. `setup/codeowl-generate.md` + global copy + `ARCHITECTURE.md` §"Feature spec shape" updated. Release binary rebuilt.

**Next: M11 corpus generation** — the user runs `/codeowl-generate --all` from a session *inside* the pilot repo (not this repo — no MCP here). See `ROADMAP.md`'s M11 "Execution / sequencing" block (added this session by a fork of the other session). Then the four cuts; BA cut needs the user.
